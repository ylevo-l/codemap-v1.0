...
from core.filesystem import FileFilter, build_tree, scan_filesystem
from core.operations import calculate_token_counts
from core.state import load_state, apply_state
...
class CodeMapApp:
    ...
    def _start_background_threads(self) -> None:
        ...
        threading.Thread(
            target=scan_filesystem,
            args=(self.root_path, self.file_filter, self.path_to_node,
                 self.tree_changed_flag, self.stop_event, self.lock),
            daemon=True
        ).start()
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--copy-format', choices=['blocks', 'lines', 'raw'], default='blocks')
    parser.add_argument('--path-mode', choices=['relative', 'basename'], default='relative')
...
def run() -> None:
    ...
    CodeMapApp(root_path, args.copy_format, args.path_mode).run()