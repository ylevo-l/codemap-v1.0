...
class CodeMap:
    ...
    def run(self):
        try:
            curses.wrapper(partial(run_application,root_node=self.root_node,path_to_node=self.path_to_node,fmt=self.copy_format,path_mode=self.path_mode,tree_changed_flag=self.tree_changed_flag,lock=self.lock))
        finally:
            self.stop_event.set()
...
def _parse_args()->argparse.Namespace:
    ...
    parser.add_argument("--copy-format",choices=["blocks","lines","raw","optimized","compact"],default="optimized",help="Format used for copying file segments.")
    parser.add_argument("--path-mode",choices=["relative","basename"],default="relative",help="Display mode for file paths.")
