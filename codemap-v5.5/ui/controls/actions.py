...
from core.processing.refactoring import refactor_file
...
class ActionHandler:
    ...
    def handle_copy_content(self, event: Event):
        ...
        if files_to_copy:
            ...
            self.ui_state.success_message = SUCCESS_MESSAGE
            return True
    ...
    def handle_refactor_content(self, event: Event):
        ...
        with self.lock:
            if self.current_node.is_dir:
                if not event.data or not event.data.get('shift', False) or not self.current_node.expanded:
                    return False

                visible_files = []
                visited = set()

                def collect_refactor_files(node: TreeNode):
                    ...
                    if node.is_dir and node.expanded:
                        for child in node.children:
                            collect_refactor_files(child)
                    elif not node.is_dir and not node.disabled:
                        visible_files.append(node.path)

                collect_refactor_files(self.current_node)
                ...
                for idx, file_path in enumerate(visible_files, 1):
                    progress_bar_length = max(10, mx - 30)
                    progress = int((idx / total_files) * progress_bar_length)
                    progress_bar = "#" * progress + "-" * (progress_bar_length - progress)
                    status = f"Refactoring {idx}/{total_files} files: [{progress_bar}]"

                    try:
                        self.stdscr.addstr(my - 1, 0, status[:mx-1], curses.color_pair(7))
                    except:
                        pass
                    self.stdscr.refresh()
                ...
                from core.processing.refactoring import refactor_directory
                from config.constants import CLEANUP_PATTERNS, CLEANUP_OPTIONS

                refactor_directory(self.current_node.path)

                self.ui_state.copying_success = True
                self.ui_state.success_message_time = time.time()
                self.ui_state.success_message = REFACTOR_ALL_SUCCESS_MESSAGE
                return True
            else:
                if refactor_file(self.current_node.path):
                    self.ui_state.copying_success = True
                    self.ui_state.success_message_time = time.time()
                    self.ui_state.success_message = REFACTOR_SUCCESS_MESSAGE
                    return True
    ...
    def register_handlers(self, control_manager):
        ...
        control_manager.register_handler(
            EventType.REFACTOR_CONTENT, lambda e: self.handle_refactor_content(e))
        control_manager.register_handler(
